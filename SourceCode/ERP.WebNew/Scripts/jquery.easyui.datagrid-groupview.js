...
var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    renderGroup: function (target, groupIndex, group, frozen) {
        ...
        // 这里原来为 if(!frozen)，为了满足我们的需求，手动改了一下，by-daniel
        if (frozen) {
        ...
        // Begin - 这部分是增加的汇总分组部分，by Daniel
        if (opts.summaryEachGroup && opts.summaryColumns) {
            for (var keyid = 0; keyid < opts.summaryColumns.length; keyid++) {
                if (opts.summaryColumns[keyid].groupFieldKey == group.value) {

                    var footerFields = $(target).datagrid("getColumnFields", opts.summaryColumns[keyid].groupedValue);

                    table.push('<table class="datagrid-ftable" cellspacing="0" cellpadding="0" border="0"><tbody>');
                    for (var i = 0; i < opts.summaryColumns[keyid].groupedValue.length; i++) {
                        table.push("<tr class=\"datagrid-row datagrid-row-groupSummary\" datagrid-row-index=\"" + i + "\">");
                        table.push(this.renderRow.call(this, target, footerFields, true, i, opts.summaryColumns[keyid].groupedValue[i]));
                        table.push("</tr>");
                    }
                    table.push('</tbody></table>');
                    break;
